...
const courses = [
    ...
    { id: 'bca', label: 'BCA' },
    { id: 'bsc', label: 'BSc' },
    // { id: 'other', label: 'Other' } // "Other" course removed
];
...
const StudyTipsSection = () => {
    ...
    const [showSubjectInput, setShowSubjectInput] = useState(false); // Control visibility of subject input
    ...
    const handleSemesterChange = (value) => {
        ...
        setSelectedSubject('');
        setShowSubjectInput(true); // Show subject input after semester selection
        setShowResourceOptions(false);
        setSelectedResource(null);
    };
    ...
    const handleSubjectChange = (e) => {
        ...
        // Show resources only if subject is not empty
        if (e.target.value.trim() !== '') {
            setShowResourceOptions(true);
        } else {
            setShowResourceOptions(false);
        }
        setSelectedResource(null); // Reset resource selection when subject changes
    };
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 pt-0 pb-12 relative z-10">
        <div className="max-w-3xl mx-auto">
          ...
          <Card className="mb-6 bg-white/80 backdrop-blur-sm shadow-lg hover:shadow-xl transition-shadow">
            ...
            <CardContent>
              <RadioGroup className="grid grid-cols-2 sm:grid-cols-4 gap-3" value={selectedCourse || ""} onValueChange={handleCourseChange}> {/* Adjusted grid-cols */}
          ...
          {selectedCourse && (<Card className="mb-6 bg-white/80 backdrop-blur-sm shadow-lg hover:shadow-xl transition-shadow animate-in fade-in">
              <CardHeader>
              ...
              <CardContent>
                <Select value={selectedSemester || ""} onValueChange={handleSemesterChange}>
                  ...
                  <SelectContent>
                    {semesters.map((semester) => (<SelectItem key={semester.value} value={semester.value}>
                        {semester.label}
                      </SelectItem>))}
                  </SelectContent>
                </Select>
            ...
            <Card className="bg-white/80 backdrop-blur-sm shadow-lg hover:shadow-xl transition-shadow animate-in fade-in">
              ...
              <CardContent className="min-h-[200px] flex items-center justify-center border-t border-gray-100">
                <div className="text-center text-gray-500">
                  <p>Resources for '{selectedSubject}' will appear here.</p>
                  <Button className="mt-4 bg-buddy-lavender hover:bg-buddy-lavender/90">Load Resources</Button>
                </div>
              </CardContent>
            </Card>
          )}
